/*
    Eko Biashara Code Library
    Main UI/UX modifiers and methods for use across modules
*/
...
var APPLICATION_MODULE = document.querySelectorAll("html")[0].getAttribute("ng-app");
var APPLICATION_MAIN_CTRL = document.querySelectorAll("html")[0].getAttribute("ng-controller");
(function(){
    angular.module(APPLICATION_MODULE, ["ngMaterial", "ui.router", "ngSanitize"]);
    angular.module(APPLICATION_MODULE).factory("$appAPI", ["$http", "$session", "$rootScope", function ($http, $session, $rootScope){
        return {
    ...
    angular.module(APPLICATION_MODULE).factory("$alert", ["$mdDialog", "$mdToast", function($mdDialog, $mdToast) {
        return {
            construct: function (htmlMessage, title, event, type, backgroundClose, confirmButtonText, dismissButtonText, onConfirm, onCancel, onDismiss) {
                htmlMessage = htmlMessage || "Alert!";
                title = title || null;
                type = type || "normal";
                title = title != null ? "<h2 style='color: inherit !important;' class='md-title'>" + title + "</h2>" : "";
                backgroundClose = "boolean" !== typeof backgroundClose ? true : backgroundClose;
                var type_color = type == "success" ? "#149138" : type == "warning" ? "#ff671f" : type == "danger" ? "#ee402f" : "#000";
                var actions = "";
                if ("string" == typeof confirmButtonText || "string" == typeof dismissButtonText){
                    actions = "<md-dialog-actions>";
                    if ("string" == typeof confirmButtonText && confirmButtonText.trim().length > 0){
                        actions += "<md-button ng-click='actionConfirm()' class='md-primary'>" + confirmButtonText + "</md-button>";
                    }
                    if ("string" == typeof dismissButtonText && dismissButtonText.trim().length > 0){
                        actions += "<md-button ng-click='actionCancel()'>" + dismissButtonText + "</md-button>";
                    }
                    actions += "</md-dialog-actions>";
                }
                var content_html = "<md-dialog><md-dialog-content style='overflow-x: hidden'><div layout='column'><div style='color: "+type_color+" !important;min-width: 300px; max-width: 350px; padding: 20px;' flex>"+ title + htmlMessage +"</div></div></md-dialog-content>"+ actions +"</md-dialog>";
                var alert = $mdDialog.alert({
                    skipHide: true,
                    template : content_html,
                    controller : function($scope, $mdDialog){
                        $scope.closeDialog = function(){
                            $mdDialog.hide("alert_close_dialot");
                            alert = null;
                        };
                    ...
                    .ariaLabel(camelCase(type) + " alert!")
                    .targetEvent(event)
                    .clickOutsideToClose(backgroundClose);
                $mdDialog.show(alert).finally(function(){
                    alert = null; if ("function" === typeof onDismiss) onDismiss();
                });
                return alert;
            },
            alert : function(title, str, event, callback) {
                this.construct(str, title, event, "normal", true, undefined, "Close", undefined, callback);
            },
            success: function (title, str, event, callback) {
                this.construct(str, title, event, "success", true, undefined, "Dismiss", undefined, callback);
            },
            warning: function (title, str, event, callback) {
                this.construct(str, title, event, "warning", true, undefined, "Ok, got it!", undefined, callback);
            },
            danger: function (title, str, event, callback) {
                this.construct(str, title, event, "danger", true, undefined, "Ok, got it!", undefined, callback);
            },
            confirm: function (title, question, confirmText, event, onConfirm, onCancel, onClose) {
                this.construct(question, title, event, "confirm", false, confirmText, "Cancel", onConfirm, onCancel, onClose);
            },
            wait: function (prompt, event, backgroundDismiss){
                var wait_text_container = document.getElementById("app-dialog-wait-text");
                if (wait_text_container != null){
                    wait_text_container.innerHTML = prompt;
                    return;
                }
                var wait_html = "<div layout='row' layout-align='space-around'>" +
                    "<md-progress-circular md-mode='indeterminate'></md-progress-circular>" +
                    "<p style='color: #585858;' id='app-dialog-wait-text'>"+ prompt +"</p>" +
                    "</div>";
                backgroundDismiss = "boolean" === typeof backgroundDismiss ? backgroundDismiss : false;
                var dialog = this.construct(wait_html, undefined, event, "wait", backgroundDismiss);
                return {
                    hide : function(){
                        if (dialog != null) $mdDialog.hide(dialog);
                    }
                }
            },
            toast: function (text, timeout, pos, actionText, onAction) {
                ...
                $mdToast.show(toast).then(function(response){
                    if (response == "ok" && "function" === typeof onAction) onAction();
                });
                return {
                    hide : function(){
                        $mdToast.hide();
    ...
    angular.module(APPLICATION_MODULE).service("$router",["$state", function($state){
        this.go = function(state, params){
            if ($state.href(state)) $state.go(state, params);
            else console.error("State not found:", state, params);
        };
    }]);
    angular.module(APPLICATION_MODULE).service("$store", function($rootScope, $appAPI, $alert, $q){
        var localStore = {
            restored : false,
            autoSave : false,
            isReady : function(){
    ...
    angular.module(APPLICATION_MODULE).provider("$session", function(){
        var settings = {
            serverURL : null,
            tokenName : "token",
    ...
    angular.module(APPLICATION_MODULE).service("$tokenAuth", ["$appAPI", "$q", "$session", function($appAPI, $q, $session){
    ...
    angular.module(APPLICATION_MODULE).controller(APPLICATION_MAIN_CTRL, ["$rootScope", "$scope", "$session", "$tokenAuth", "$router", "$alert", function($rootScope, $scope, $session, $tokenAuth, $router, $alert){
        ...
        $rootScope.lockSession = function(event){
            ...
            if ("undefined" === typeof event || event === null){
                ...
                $alert.confirm("Lock Session", "Are you sure you want to lock this session?", "Yes", event, function(){
                    lock();
                });
            }
        };
        $rootScope.logoutSession = function(event, redirectToLogin){
            $alert.confirm("Logout", "Are you sure you want to sign out this session?", "Yes", event, function(){
                $rootScope.session = null;
    ...
    angular.module(APPLICATION_MODULE).config(["$locationProvider", "$urlRouterProvider", "$stateProvider", function($locationProvider, $urlRouterProvider, $stateProvider){
        $locationProvider.html5Mode({enabled:true, requireBase:false});
        $urlRouterProvider.otherwise("/redirecting");
        $stateProvider.state("redirecting", {
            url: "/redirecting",
            template: "",
            controller: ["$rootScope", "$scope", "$session", "$router", "$tokenAuth", function ($rootScope, $scope, $session, $router, $tokenAuth){
                $rootScope.setTitle($session.settings.redirectingTitle);
                $rootScope.setMainLoading(true);
                var redirect = function(){
                    if ($rootScope.isAuthenticated()) $router.go($session.settings.sessionState, $session.settings.sessionStateParams);
                    else $router.go($session.settings.defaultHomeState, $session.settings.defaultHomeStateParams);
                };
                if ($tokenAuth.isAuthenticating()){
                    $rootScope.$on("token_auth_complete", function(){
                        redirect();
                    });
                }
